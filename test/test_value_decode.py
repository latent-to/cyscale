...
class TestValueDecodeEquivalence(unittest.TestCase):
    ...
    def test_value_decoder_engaged(self):
        ...
        for ts in ('u32', 'Vec<u32>', 'Option<u64>', '(u8, u16)', '[u8; 4]',
                   'Compact<u32>', 'AccountId', 'bool', 'Vec<u8>'):
    ...
    def test_vec(self):
        self.assertEqual(self.roundtrip('Vec<u32>', [1, 2, 3, 2**32 - 1]), [1, 2, 3, 2**32 - 1])
        self.assertEqual(self.roundtrip('Vec<u32>', []), [])
        self.assertEqual(
            self.roundtrip('Vec<Option<bool>>', [True, None, False]), [True, None, False]
        )
...
class TestValueDecodeScaleInfo(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.runtime_config = RuntimeConfigurationObject(ss58_format=42, implements_scale_info=True)
    ...
    def test_registry_wide_equivalence(self):
        ...
        registry = self.metadata_obj.portable_registry.value_object['types'].value_object
        ...
        self.assertGreater(supported, 100, 'value decoder supports suspiciously few types')
