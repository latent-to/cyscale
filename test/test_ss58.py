...
class SS58TestCase(unittest.TestCase):
    ...
    def test_encode_8_bytes_account_index(self):
        self.assertEqual(
            "848Gh2GcGaZia", ss58_encode("0x2a2c0a0000000000", ss58_format=2)
        )
    ...
    def test_encode_subkey_generated_pairs(self):
        for subkey_pair in self.subkey_pairs:
            self.assertEqual(
                subkey_pair["address"],
                ss58_encode(
                    address=subkey_pair["public_key"],
    ...
    def test_decode_subkey_generated_pairs(self):
        for subkey_pair in self.subkey_pairs:
            self.assertEqual(
                ...
                "0x"
                + ss58_decode(
                    address=subkey_pair["address"],
                    valid_ss58_format=subkey_pair["ss58_format"],
                ),
            )
    ...
    def test_invalid_ss58_format_check(self):
        with self.assertRaises(ValueError) as cm:
            ss58_decode(
                "5GoKvZWG5ZPYL1WUovuHW3zJBWBP5eT8CbqjdRY4Q6iMaQua", valid_ss58_format=2
            )
    ...
    def test_is_valid_ss58_address(self):
        ...
        self.assertTrue(
            is_valid_ss58_address(
                "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY", valid_ss58_format=42
            )
        )
        self.assertFalse(
            is_valid_ss58_address(
                "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY", valid_ss58_format=2
            )
        )

        self.assertTrue(
            is_valid_ss58_address(
                "GLdQ4D4wkeEJUX8DBT9HkpycFVYQZ3fmJyQ5ZgBRxZ4LD3S", valid_ss58_format=2
            )
        )
        self.assertFalse(
            is_valid_ss58_address(
                "GLdQ4D4wkeEJUX8DBT9HkpycFVYQZ3fmJyQ5ZgBRxZ4LD3S", valid_ss58_format=42
            )
        )
        self.assertFalse(
            is_valid_ss58_address(
                "GLdQ4D4wkeEJUX8DBT9HkpycFVYQZ3fmJyQ5ZgBRxZ4LD3S", valid_ss58_format=0
            )
        )
        self.assertTrue(
            is_valid_ss58_address(
                "12gX42C4Fj1wgtfgoP624zeHrcPBqzhb4yAENyvFdGX6EUnN", valid_ss58_format=0
            )
        )

        self.assertFalse(
            is_valid_ss58_address("5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQ")
        )
        self.assertFalse(
            is_valid_ss58_address("6GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY")
        )
        self.assertFalse(
            is_valid_ss58_address(
